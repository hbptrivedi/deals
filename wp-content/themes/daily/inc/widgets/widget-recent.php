 ...
 * @since      1.0.0
 */
class Daily_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-daily-recent posts-thumbnail-widget',
			'description' => __( 'Display recent posts with thumbnails.', 'daily' )
		);

		// Create the widget.
		parent::__construct(
			'daily-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts Thumbnails', 'daily' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'daily_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'daily_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'daily_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'daily' ),
			'limit'     => 5,
			'show_date' => false,
			'layout'    => 'standard'
		);
