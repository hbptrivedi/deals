 ...
 * @return array
 */
function daily_entry_markup( $classes ) {
...
add_filter( 'post_class', 'daily_entry_markup' );
 ...
 * @return string
 */
function daily_comments_popup_link_attributes( $attr ) {
	return ' itemprop="discussionURL"';
}
add_filter( 'comments_popup_link_attributes', 'daily_comments_popup_link_attributes', 5 );
 ...
 * @return string
 */
function daily_get_avatar( $avatar ) {
	return preg_replace( '/(<img.*?)(\/>)/i', '$1itemprop="image" $2', $avatar );
}
add_filter( 'get_avatar', 'daily_get_avatar', 5 );
 ...
 * @return array
 */
function daily_img_attr( $attr ) {
    $attr['itemprop'] = 'image';
    return $attr;
}
add_filter( 'wp_get_attachment_image_attributes', 'daily_img_attr', 10, 2 );