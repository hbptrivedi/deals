<?php get_header(); ?>
	...
	<?php $layout = of_get_option( 'daily_home_layout', 'standard' ); ?>
		
	<section id="primary" class="content-area column">

		<?php if( of_get_option('daily_home_featured') == 'slider' ) { 
			
			daily_featured_slider(); ?>
				
		<?php } elseif(of_get_option('daily_home_featured') == 'classic') { 
			
			daily_featured_posts(); ?>

		<?php } elseif(of_get_option('daily_home_featured') == 'disable') { ?>
		...
		<main id="main" class="content-loop category-box <?php echo esc_attr( daily_archive_page_classes() ); ?> column" role="main" <?php hybrid_attr( 'content' ); ?>>
			...
			<?php if ( have_posts() ) : ?>
				...
				<header class="page-header">
					<h3 class="widget-title"><strong><?php echo of_get_option( 'daily_home_title'); ?></strong></h3>
				</header><!-- .page-header -->
